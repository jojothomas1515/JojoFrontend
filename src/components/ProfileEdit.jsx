...
const ProfileEdit= forwardRef((props,ref)=> {
    ...
    useEffect(()=>{
        image_input_ref.current.addEventListener('change', ()=>{
            ...
            // console.log(image_tag_ref.current.src)
        })
    }, [])
    ...
    return (
        <div className={'contain edit-profile'} ref={ref} >
            <form action={''} method={'POST'}>
                ...
                <div><img src="JOJO.JPG" alt="profile image" ref={image_tag_ref}/><label
                    htmlFor="profile_image"><FontAwesomeIcon icon={faImage}/></label><input style={{display:'none'}}  ref={image_input_ref} type="file" name="profile_image"
                                                           id="profile_image"/>
                </div>
